...
local function pressure(v)-- returns the presure at a point (v) on the sublevel.
    local o = sublevel.getLastPos().position-- gets the position of the computer relitive to the world, not the sublevel.
...
local function computeForces(profile, T)
    local data  = { {}, {}, {} }
    for i = 1, #profile do
        local P = profile[i]
        local ai = p.facing
